...
// INFO ------------------------------------------------------------------------
// level - уровень воды     (from Table  water_level)
// array - берега           (from Table  river_board_new)
// river - ГИС              (from Table  build_river)
//------------------------------------------------------------------------------
...
$row    = mysql_fetch_array($result);
    ...
    //exit;
    //}
$level = 150;//$row['level'];                                                         // уровень воды (Table :: water_level)
...
{
    $array[] = ['x'=>$row['x'], 'y1'=>$row['y1'], 'y2'=>$row['y2']];            // берега (from Table river_board_new)
}
...
while ($row = mysql_fetch_array($result))
{
    ...
    $river[$row['x']][] = ['y'=>$row['y'], 'z'=>$row['z']];                     // gis коррдинаты (from Table build_river)
...
$draw1=[];                                                                      //нижний берег РУЧНОЙ
$draw2=[];                                                                      //верхний берег РУЧНОЙ
$draw3=[];                                                                      //нижний берег ГИС
$draw4=[];                                                                      //верхний берег ГИС
$draw5=[];                                                                      //нижний берег ПОДТОПЛЕНИЕ
$draw6=[];                                                                      //верхний берег ПОДТОПЛЕНИЕ


// ---------Досліжувана область//


// ЛИНИИ БЕРЕГОВ (РУЧНОЙ)-------------------------------------------------------
...
// Опеределение ГИС-координат берегов ------------------------------------------
